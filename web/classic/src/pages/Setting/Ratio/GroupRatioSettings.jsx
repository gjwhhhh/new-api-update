...
export default function GroupRatioSettings(props) {
  ...
  async function onSubmit() {
    ...
    setLoading(true);
    try {
      const currentRes = await API.get('/api/group/config');
      if (!currentRes.data?.success || !currentRes.data?.data) {
        return showError(currentRes.data?.message || t('加载分组配置失败'));
      }

      const { revision, config } = currentRes.data.data;
      const nextConfig = {
        ...config,
        group_ratio: JSON.parse(inputs.GroupRatio),
        user_usable_groups: JSON.parse(inputs.UserUsableGroups),
        group_group_ratio: JSON.parse(inputs.GroupGroupRatio),
        auto_groups: JSON.parse(inputs.AutoGroups),
        default_use_auto_group: !!inputs.DefaultUseAutoGroup,
        group_special_usable_group: JSON.parse(
          inputs['group_ratio_setting.group_special_usable_group'],
        ),
      };
      const res = await API.put('/api/group/config', {
        revision,
        config: nextConfig,
      });
      if (!res.data?.success) {
  ...
  const openRename = useCallback((name) => {
  ...
  const previewRename = useCallback(async () => {
    try {
      const res = await API.post('/api/group/rename-preview', {
        old_name: renameTarget,
        new_name: renameName.trim(),
      });
      if (!res.data?.success || !res.data?.data) {
        showError(res.data?.message || t('预检失败'));
        return;
      }
      setRenamePreview(res.data.data);
    } catch {
      showError(t('预检失败'));
    }
  ...
  const executeRename = useCallback(async () => {
    try {
      const res = await API.post('/api/group/rename', {
        old_name: renameTarget,
        new_name: renamePreview?.new_name,
        revision: renamePreview?.revision,
      ...
      showSuccess(t('分组已重命名'));
      setRenameTarget('');
      props.refresh();
    } catch {
      showError(t('重命名失败'));
    }
  }, [props, renameConfirmation, renamePreview, renameTarget, t]);
    ...
    <Spin spinning={loading}>
      ...
      <Modal
        ...
        footer={
          renamePreview ? (
            <>
              <Button onClick={() => setRenameTarget('')}>{t('取消')}</Button>
              <Button
                theme='solid'
                type='danger'
                disabled={renameConfirmation !== renameTarget}
        ...
        <div className='space-y-3'>
          ...
          {renamePreview && (
            <>
              <Text type='tertiary'>
                {t(
                  '将迁移 {{users}} 个用户、{{tokens}} 个令牌、{{channels}} 个渠道',
                  {
                    users: renamePreview.affected.users,
                    tokens: renamePreview.affected.tokens,
                    channels: renamePreview.affected.channels,
                  },
                )}
              </Text>
              <Input
                value={renameConfirmation}
                placeholder={t('输入原分组名称以确认：{{name}}', {
                  name: renameTarget,
                })}
                onChange={setRenameConfirmation}
              />
            </>
          )}
        </div>
