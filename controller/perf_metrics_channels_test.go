...
func TestGetPerfMetricsChannelsReturnsSafeChannelMetrics(t *testing.T) {
	...
	require.NoError(t, model.DB.Where("channel_id = ?", channelID).Delete(&model.PerfChannelMetric{}).Error)
	t.Cleanup(func() {
		_ = model.DB.Where("id = ?", channelID).Delete(&model.Channel{}).Error
		_ = model.DB.Where("channel_id = ?", channelID).Delete(&model.PerfChannelMetric{}).Error
	})
	...
	ctx.Request = httptest.NewRequest(
		http.MethodGet,
		"/api/channel/status-metrics?hours=48&search="+channelName,
		nil,
	)
